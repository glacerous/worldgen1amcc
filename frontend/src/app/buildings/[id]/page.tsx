...
export default function BuildingDetailPage({
  ...
  useEffect(() => {
    ...
    setIsAdminLoggedIn(!!adminTok);
  }, []);

  const executeDeleteAudit = async () => {
    const adminTok = sessionStorage.getItem("admin_token");
    const activeToken = adminTok || token;
    if (!selectedRunId || !activeToken) return;

    setIsDeletingAudit(true);
    setDeleteError(null);
    try {
      const deleteEndpoint = adminTok
        ? `${BACKEND_URL}/admin/audit-runs/${selectedRunId}`
        : `${BACKEND_URL}/audit-runs/${selectedRunId}`;
            ...
            {!loadingRuns && auditRuns.length > 0 && selectedRun && (
              <div className="text-xs font-sans mt-1">
                ...
                <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1.5 text-ink-muted">
                  ...
                  {/* Edit & Delete buttons for current run if owned by logged-in user or if admin */}
                  {(isAdminLoggedIn || (user?.id && selectedRun.user_id === user.id)) && (
                    <div className="inline-flex items-center gap-1.5">
                      {((user?.id && selectedRun.user_id === user.id) || isAdminLoggedIn) && (
                        <Link
